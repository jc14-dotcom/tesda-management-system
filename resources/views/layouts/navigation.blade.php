    ...
    <div class="page-container">
        <div class="flex h-16 items-center">
            ...
            <div class="flex flex-1 items-center justify-end gap-2">
                ...
                <!-- Notification Bell + Panel (desktop + mobile) -->
                <div class="relative" id="notif-panel-wrapper">
                    ...
                    <div
                        id="notif-panel"
                        class="fixed left-4 right-4 top-[4.25rem] z-50 hidden overflow-hidden rounded-card border border-grayTheme-border bg-white shadow-modal sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-[22rem]"
                        role="dialog"
                        aria-label="Notifications"
                    >
                        {{-- Header --}}
                        ...
                        <div id="notif-list" class="max-h-[20rem] divide-y divide-grayTheme-border overflow-y-auto">
                            @include('layouts.partials.notif-list-items', ['recentNotifications' => $recentNotifications, 'notificationsIndexUrl' => $notificationsIndexUrl])
                        </div>
                ...
                <div class="-me-2 flex items-center gap-1 sm:hidden">
                    <!-- Profile (mobile) -->
                    <a href="{{ route('account.profile') }}" class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-grayTheme-border bg-white shadow-card transition hover:border-primary hover:bg-primary-soft" aria-label="My Profile">
                        @if ($photoUrl ?? false)
                            <img src="{{ $photoUrl }}" alt="{{ Auth::user()->name }}" class="h-full w-full rounded-full object-cover" />
                        @else
                            @php
                                $mobileInitials = collect(explode(' ', Auth::user()->name))
                                    ->map(fn($w) => mb_strtoupper(mb_substr($w, 0, 1)))
                                    ->take(2)
                                    ->implode('');
                            @endphp
                            <span class="flex h-full w-full items-center justify-center rounded-full bg-primary text-xs font-bold text-white">{{ $mobileInitials }}</span>
                        @endif
                    </a>
